...
export default function setupCollaboratorsRESTEndpoints(ctx: IBaseContext, app: Express) {
  const endpoints = {
    getOrganizationCollaborators: wrapEndpointREST(getOrganizationCollaborators, ctx),
    removeCollaborator: wrapEndpointREST(removeCollaborator, ctx),
  };

  app.post(`${baseURL}/getOrganizationCollaborators`, endpoints.getOrganizationCollaborators);
