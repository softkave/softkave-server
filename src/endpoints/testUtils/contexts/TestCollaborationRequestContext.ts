...
class TestCollaborationRequestContext implements ICollaborationRequestContext {
  ...
  assertGetCollaborationRequestById = async (ctx: IBaseContext, id: string) => {
    const request = await ctx.collaborationRequest.getCollaborationRequestById(
      ctx,
      id
    );
  ...
  public deleteCollaborationRequestById = async (
    ...
    const index = this.requests.findIndex((request) => request.customId === id);
  ...
  public changeRequestsRecipientEmail = async (
    ctx: IBaseContext,
    fromEmail: string,
    toEmail: string
  ) => {
    ...
    this.requests.forEach((request) => {
      if (request.to.email === fromEmail) {
        request.to.email = toEmail;
      }
    });
  };
  ...
  public getCollaborationRequestsByBlockId = async (
    ...
    return this.requests.filter((request) => request.from.blockId === blockId);
  };
