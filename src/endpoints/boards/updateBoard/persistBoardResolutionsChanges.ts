import {IBoard, IBoardStatusResolution} from '../../../mongo/block/board';
...
function getResolutionChanges(
  ...
  return ['description', 'name'].reduce((accumulator, field) => {
    const f = field as keyof IBoardStatusResolution;
    if (oldItem[f] !== newItem[f]) {
      accumulator.push(f);
    }

    return accumulator;
  }, [] as Array<keyof IBoardStatusResolution>);
}
...
async function persistBoardResolutionsChanges(
  ...
  boardResolutions.forEach(resolution => {
    const existingResolution = indexedOldBoardResolutions[resolution.customId];
    if (!existingResolution) {
      return;
    }

    if (existingResolution.updatedAt !== resolution.updatedAt) {
      const changedFields = getResolutionChanges(existingResolution, resolution);
      const newValue: any = {};
      const oldValue: any = {};
  ...
  fireAndForgetPromise(
    context.bulkUpdateDeletedResolutionsInTasks(context, board.customId, deletedResolutionIds)
  );
}
