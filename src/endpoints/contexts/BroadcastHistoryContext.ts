...
export interface IBroadcastHistoryContext {
  insert: (ctx: IBaseContext, room: string, item: IBroadcastHistoryItem) => void;
  fetch: (ctx: IBaseContext, from: string, rooms: string[]) => IBroadcastHistoryFetchResult;
}
...
export default class BroadcastHistoryContext implements IBroadcastHistoryContext {
  insert = (ctx: IBaseContext, room: string, item: IBroadcastHistoryItem) => {
  ...
  fetch = (ctx: IBaseContext, from: string, rooms: string[]) => {
    ...
    for (const room of rooms) {
      ...
      if (roomTimestamps[roomTimestamps.length - 1].timestamp < fromDate) {
        continue;
      }

      // TODO: how can we binary search here, if it's faster
      let closestTimestampIndex = findLastIndex(roomTimestamps, item => {
...
export const getBroadcastHistoryContext = makeSingletonFn(() => new BroadcastHistoryContext());
