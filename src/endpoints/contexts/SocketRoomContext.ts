...
export class SocketRoomContext implements ISocketRoomContext {
  ...
  addToRoom(
    ...
    if (roomCreationOptions?.useSocketIdsFromRoom) {
      this.rooms[roomCreationOptions.useSocketIdsFromRoom].roomsUsingSocketIdsFromRoom[
        roomName
      ] = 1;

      this.internalAddSocketToRoom(roomCreationOptions.useSocketIdsFromRoom, socketId);
    } else {
  ...
  broadcastToRoom(
    ...
    if (!room || (room.useSocketIdsFromRoom && !this.rooms[room.useSocketIdsFromRoom])) {
      return;
    }
    ...
    if (Object.keys(socketIds).length === 0) {
      this.internalRemoveRoom(room.useSocketIdsFromRoom ? room.useSocketIdsFromRoom : room.name);
    } else {
  ...
  private internalRemoveRoom(roomName: string) {
    ...
    Object.keys(room.roomsUsingSocketIdsFromRoom).forEach(name => this.internalRemoveRoom(name));
  }
